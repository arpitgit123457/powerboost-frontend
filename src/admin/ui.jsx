...
export function Card({ className = '', children }) {
  return (
    <div className={`rounded-2xl border border-line bg-card p-6 shadow-sm ${className}`}>
      {children}
    </div>
  )
}
...
export function Input({ label, ...props }) {
  return (
    <label className="block">
      ...
      <input
        {...props}
        className="w-full rounded-xl border border-line bg-surface px-4 py-2.5 text-sm text-ink outline-none transition placeholder:text-mist/60 focus:border-accent/60 focus:bg-card"
      />
    </label>
  )
}
...
export function Select({ label, children, ...props }) {
  return (
    <label className="block">
      ...
      <select
        {...props}
        className="w-full rounded-xl border border-line bg-surface px-4 py-2.5 text-sm text-ink outline-none transition focus:border-accent/60 focus:bg-card"
      >
        {children}
      </select>
    </label>
...
export function Btn({ children, variant = 'primary', className = '', ...props }) {
  const styles = {
    ...
    outline: 'border border-line bg-card text-ink hover:border-accent/50 hover:text-accent',
...
export function Modal({ open, title, onClose, children }) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-2xl rounded-3xl bg-card shadow-lux"
